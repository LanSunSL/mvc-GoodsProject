...
public interface IBaseDAO<K, V> {
	 ...
	 * @param pageSize 每行显示的数据长度，这个长度也就决定了ArrayList初始化的长度
	 * @return 由于返回多行的查询记录，所以每行记录都要求封装在VO类对象之中，如果没有返回任何查询结果，该集合长度为0(size()==0)，而不是null
	 * @throws SQLException 通过PreparedStatement执行操作返回的是SQL异常，看见SQL异常就查数据层代码
	 */
	public List<V> findAllSplit(Integer currentPage,Integer pageSize) throws SQLException ;
	/**
	 * 数据模糊的分页查询操作
	 ...
	 * @throws SQLException 通过PreparedStatement执行操作返回的是SQL异常，看见SQL异常就查数据层代码
	 */
	public List<V> findAllSplit(String column,String keyWord, Integer currentPage,Integer pageSize) throws SQLException ;
	/**
